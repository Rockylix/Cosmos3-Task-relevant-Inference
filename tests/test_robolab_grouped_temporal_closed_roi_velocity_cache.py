...
def test_strategy_version_is_v5() -> None:
    assert VELOCITY_CACHE_STRATEGY_VERSION == "v5"


def test_masks_keep_frames_separate_apply_temporal_closure_and_nest_depth() -> None:
    scores, raw, closed, execution = build_grouped_temporal_closed_masks(
        torch=torch,
        profile_records=_records(),
        block_groups=((4, 5), (6, 7), (8, 9)),
        threshold=0.9,
    )
    assert scores.shape == raw.shape == closed.shape == execution.shape == (3, 8, 4)
    assert torch.equal(raw[0, 0], torch.tensor([True, False, False, False]))
    assert torch.equal(raw[0, 1], torch.tensor([False, True, False, False]))
    assert torch.equal(closed[0, 0], torch.tensor([True, True, False, False]))
    assert torch.equal(closed[0, 1], torch.tensor([True, True, True, False]))
