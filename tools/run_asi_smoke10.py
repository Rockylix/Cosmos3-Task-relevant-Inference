...
def aggregate(run, phase):
    ...
    if len(paired) == 10:
        ...
        value["speedup"] = value["timing"]["dense"]["median_s"] / value["timing"]["asi"]["median_s"]
        value["fidelity"] = {
            key: {
...
def main():
    ...
    source_files = [
        ...
        "cosmos_framework/scripts/asi_smoke.py",
    ]
    write_json(
        run / "manifest.json",
        {
            ...
            "strategy": "edge-core80-stable104-action-weighted",
            "schedule": "step0 conditional dense profile; seven remaining CFG stacks sparse",
            "profile_blocks": list(range(28)),
            "core_block_count": 6,
            "core_token_budget": 80,
            "stable_token_budget": 104,
            "action_horizon_weights": [1 / 6, 1 / 3, 1 / 3, 1 / 6],
            "mask_refresh": "every request; one mask across all blocks and remaining CFG stacks",
            "cached_velocity": False,
            "simulator_seed": 0,
            "policy_seed": 0,
    ...
    common = [
        ...
        "-u",
        "-m",
        "cosmos_framework.scripts.asi_smoke",
        "--eval-root",
        str(run),
        "--gate-capture",
        str(GATE),
    ...
    try:
        ...
        while "[asi-server] READY" not in (run / "server.log").read_text(errors="replace"):
            if server.poll() is not None or time.monotonic() > deadline:
                raise RuntimeError("Server/GPU gate failed; see server.log")
            time.sleep(2)
        print("[ASI] GPU gate passed; starting 10 episodes (8 simple + 2 moderate)", flush=True)
        ...
        while simulator.poll() is None:
            ...
            if val["completed"] != last:
                ...
                print(
                    f"[ASI] {last}/10 completed | success {val['success_count']}/{last} | score {score}",
                    flush=True,
                )
            time.sleep(5)
        ...
        stop_owned(server)
        print("[ASI] 10 episodes finished; simulator stopped; paired GPU-only warm timing starts", flush=True)
        ...
        while benchmark.poll() is None:
            ...
            if val["paired_tasks"] != last:
                ...
                print(f"[ASI] paired timing {last}/10 tasks", flush=True)
            time.sleep(5)
