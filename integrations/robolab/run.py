...
import cv2  # noqa: F401 - Must import before isaaclab for simulator initialization.
...
from c3ache_client import C3acheCosmos3Client as Cosmos3Client
...
def make_client(args: argparse.Namespace) -> Cosmos3Client:
    """Create the isolated C3ache client while preserving the native evaluator."""
    return Cosmos3Client(remote_host=args.remote_host, remote_port=args.remote_port)


def main() -> None:
    """Run the unchanged RoboLab evaluation and close its simulation application."""
